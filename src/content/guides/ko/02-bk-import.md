# BlueKiwi 실전 튜토리얼 #02 — bk-import: 외부 자료를 워크플로로 변환
...
4. 브라우저에서 바로 확인하고 `/bk-start`로 실행
...
| 로컬 스킬 이름         | `bk-create`, `bk-start`         | `~/.claude/skills/<name>/SKILL.md` 읽기 |
...
`~/.claude/skills/bk-create/SKILL.md`를 읽어 해당 스킬의 실행 흐름을 BlueKiwi 워크플로로 변환합니다.
...
`/bk-import`는 `/bk-create`과 동일한 **Instruction Depth Standard**를 적용합니다. 각 노드 지침에는 반드시:
...
입력 타입을 선택하는 질문에서 "Paste text or JSON"을 고르고 JSON을 입력하면, n8n의 노드 구조(Trigger → HTTP Request → If → Set → ...)를 BlueKiwi action/gate/loop로 매핑합니다.
...
- **로컬 스킬 이름**은 `/bk-` 접두사 없이도 인식합니다 (`create` → `bk-create`로 자동 해석).
