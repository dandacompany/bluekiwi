...
BlueKiwi는 **AI 코딩 에이전트를 위한 셀프호스팅 워크플로우 엔진**입니다. 웹 UI에서 멀티스텝 워크플로우를 한 번 설계하면, 연결된 에이전트(Claude Code, Codex CLI, Gemini CLI 등)가 언제든 실행할 수 있습니다 — 모든 단계가 실시간 타임라인으로 기록됩니다.
...
최초 설정 시 기본 워크플로우 예제와 함께 Open Design을 참고한 full component kit 3종이 자동 설치됩니다.

| Seed                        | 용도                                                |
| --------------------------- | --------------------------------------------------- |
| `Shadcn Product UI Kit`     | 범용 React/Tailwind/shadcn 제품 UI                  |
| `Kami Editorial Paper Kit`  | 따뜻한 문서, 강의, 화이트페이퍼, 슬라이드 보조 화면 |
| `Dashboard Operations Dark` | 고밀도 다크 분석, 모니터링, 인시던트 대시보드       |
...
`bluekiwi` MCP 서버는 에이전트 런타임이 자동으로 호출하는 도구를 제공합니다. 전체 레퍼런스는 실행 중인 서버의 **`/docs`** (Swagger UI + OpenAPI JSON)에서 확인하세요.
...
| 커맨드                                         | 설명                                          |
| ---------------------------------------------- | --------------------------------------------- |
| `bluekiwi accept <token> --server <url>`       | 팀 초대 수락 및 에이전트 런타임 설정          |
| `bluekiwi init --server <url> --api-key <key>` | 기존 API 키로 직접 연결 (슈퍼유저 / 관리자)   |
...
| 18  | Hermes Agent            | `hermes`         | `~/.hermes/config.yaml` — 프로필은 `hermes:<profile>`로 자동 감지 (`~/.hermes/profiles/<name>/config.yaml`)                 | YAML (`mcp_servers:` + sentinel 블록) |

연결 완료 후 BlueKiwi는 내장 스킬을 각 런타임의 스킬 디렉토리에 복사합니다. 네이티브로 스킬을 자동 스캔하는 런타임(Claude Code, Claude Desktop, Codex, Gemini, OpenCode, OpenClaw, Hermes)에서는 슬래시 커맨드가 즉시 활성화되고, 네이티브 지원이 없는 IDE에서는 `<base>/skills/` 경로에 파일만 기록(향후 호환용).
...
앱 시작 시 마이그레이션이 자동으로 실행됩니다. 신규 설치의 경우 `scripts/migrate.js`가 `docker/migrations/`의 모든 SQL 파일을 순서대로 적용하고 `schema_migrations` 테이블에 기록합니다 — 별도의 수동 작업이 필요하지 않습니다.
