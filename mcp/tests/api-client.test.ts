...
import { BlueKiwiAuthError, BlueKiwiApiError } from "../src/errors.js";
...
describe("BlueKiwiClient", () => {
  ...
  it("sends bearer token and returns parsed JSON", async () => {
    ...
    const result = await client.request<{ ok: boolean }>("GET", "/api/workflows");
  ...
  it("throws BlueKiwiAuthError on 401", async () => {
    ...
    await expect(client.request("GET", "/api/workflows")).rejects.toBeInstanceOf(
      BlueKiwiAuthError
    );
  });
