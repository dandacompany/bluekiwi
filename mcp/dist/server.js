...
async function resolveInstruction(node) {
...
async function resolveCredential(node) {
...
function maskSecrets(secretsJson) {
  try {
...
function extractVersionNumber(segment) {
...
function getLatestChain(chains) {
...
server.tool(
  ...
  async ({ chain_id, version, context, session_meta }) => {
    const { rows: requestedChainRows } = await pool.query(
...
server.tool(
  ...
  async ({ task_id, peek }) => {
    ...
    if (peek) {
      ...
      const currentNode = currentNodeRows[0];
      let currentLog;
      if (currentNode) {
        const { rows: logRows } = await pool.query(
  ...
  async ({ chain_id, title, description, evaluation_contract, nodes }) => {
    ...
    try {
      ...
      if (nodes) {
        ...
        const { rows: insertedChainRows } = await client.query(
          "INSERT INTO chains (title, description, version, parent_chain_id, evaluation_contract) VALUES ($1, $2, $3, $4, $5::jsonb) RETURNING id, title, description, version, parent_chain_id, evaluation_contract",
          [
            nextTitle,
            nextDescription,
        ...
        const { rows: updatedChainRows } = await client.query(
